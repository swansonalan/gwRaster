...
#' @export
gwr_fit_map <- function(x,img.fn,verbose=T,overwrite=F,overwrite.resamps=F,do.stats=F){
  t0 <- unclass(Sys.time())
  ...
  if(np>1){
    ...
    if(inherits(try(terra::rast(img.fn2)),"try-error")) {
      print(img.fn2)
      stop(paste(c("one or more raster inputs appear to be of the wrong extent/projection, or are missing, or are corrupt",img.fn2),collapse=" "))
    }
    ...
    out.dir <- paste0(dirname(img.fn[1]),"/gwr_fits/")
    out.name <- paste0(out.dir,x@name,"_preds.tif")
    if(!file.exists(coef.name)) stop(paste(coef.name,"doesn't exist"))
    if(file.exists(out.name) & overwrite) file.remove(out.name)
    if(!file.exists(out.name)){
        t1 <- unclass(Sys.time())
        arg <- paste(get_proj_string(coef.name,img.fn2[1],band=1),out.name)
        if(verbose) cat("calculating final raster using bilinear interpolation\n")
        system(arg)
        cat("done\n\n")
    } else {
      if(verbose) cat(out.name,"already made, re-using\n")
  ...
  t2 <- unclass(Sys.time())
  if(do.stats)
  x@rdata$rast.fit <- if(do.stats) textract(terra::rast(out.name), x@rdata) else NA
